...
void readIR(uint8_t times){
  ...
  if(distance_TL != 0 || distance_TR != 0 ||  distance_L != 0 || distance_R != 0){
  ...
  else if(distance_TL == 0 && distance_TR == 0 && distance_L == 0 && distance_R == 0){      // If not then reset the arrays
    ...
    for(uint8_t x = 1; x < distanceArrayCounter; x++){
      ...
      switch(expectedGesture){
        case 0:                   // If "expectedGesture" is 0 then determine the expected gesture 
          ...
          if((increasing_TR && !increasing_TL) ||  (increasing_R && !increasing_L)){         // Left to right movement
            expectedGesture = 1;  // Expected an "east" gesture
          }
          else if((!increasing_TR && increasing_TL) ||  (!increasing_R && increasing_L)){    // Right to left movement
            expectedGesture = 2;  // Expected a "west" gesture
          }
          else if(increasing_TR && increasing_TL ){    // Top to bottom movement
            expectedGesture = 3;  // Expected a "south" gesture
          }
          else if( !increasing_TL && !increasing_TR){    // Bottom to top movement
          ...
          else if(distance_TL - distanceArray_TL[2] > 50 && distance_TR - distanceArray_TR[2] > 50){  // Up to down movementd
        ...
        case 1:   
          ...
          if((increasing_TR && !increasing_TL) ||  (increasing_R && !increasing_L)){
...
void processHoldGesture(){
  ...
  if(-20 <= distance_TR - lastDistanceHold_TR && distance_TR - lastDistanceHold_TR <= 20 && -20 <= distance_TL - lastDistanceHold_TL && distance_TL - lastDistanceHold_TL <= 20 &&  -20 <= distance_R - lastDistanceHold_R && distance_R - lastDistanceHold_R <= 20 && -20 <= distance_L - lastDistanceHold_L && distance_L - lastDistanceHold_L <= 20){
    if(distance_TR != 0 || distance_TL != 0  || distance_L != 0 || distance_R != 0  ){
      switch(gesture){
        case 1:
...
void readIR2(uint8_t times){
  ...
  if(distance_TL2 != 0 || distance_TR2 != 0 || distance_L2 != 0 || distance_R2 != 0){
  ...
  else if(distance_TL2 == 0 && distance_TR2 == 0 &&  distance_L2 == 0 && distance_R2 == 0){      // If not then reset the arrays
    readGesture2 = true;
    ...
    for(uint8_t x = 1; x < distanceArrayCounter2; x++){
      ...
      switch(expectedGesture2){
        case 0:                   // If "expectedGesture2" is 0 then determine the expected gesture2 
                                  // by analyzing if the sensors' values are increasing or decreasing
          if((increasing_TR2 && !increasing_TL2) ||  (increasing_R2 && !increasing_L2)){         // Left to right movement
            expectedGesture2 = 1;  // Expected an "east" gesture2
          }
          else if((!increasing_TR2 && increasing_TL2) ||  (!increasing_R2 && increasing_L2)){    // Right to left movement
            expectedGesture2 = 2;  // Expected a "west" gesture2
          }
          else if(increasing_TR2 && increasing_TL2 ){    // Top to bottom movement
            expectedGesture2 = 3;  // Expected a "south" gesture2
          }
          else if( !increasing_TL2 && !increasing_TR2){    // Bottom to top movement
            expectedGesture2 = 4;  // Expected a "north" gesture2
          }
          // Check if the current "distance_xx" minus the 2nd element in the array is greater than 50 then it is a "down" movement.
          // The "increasing_xx" vars are not used because they are very precise and they didn´t detect the "down" movement most of the time.
          else if(distance_TL2 - distanceArray_TL2[2] > 50 && distance_TR2 - distanceArray_TR2[2] > 50){  // Up to down movementd
        ...
        case 1:   
          ...
          if((increasing_TR2 && !increasing_TL2)  || (increasing_R2 && !increasing_L2)){
...
void processHoldgesture2(){
  ...
  if(-20 <= distance_TR2 - lastDistanceHold_TR2 && distance_TR2 - lastDistanceHold_TR2 <= 20 && -20 <= distance_TL2 - lastDistanceHold_TL2 && distance_TL2 - lastDistanceHold_TL2 <= 20 && -20 <= distance_R2 - lastDistanceHold_R2 && distance_R2 - lastDistanceHold_R2 <= 20 && -20 <= distance_L2 - lastDistanceHold_L2 && distance_L2 - lastDistanceHold_L2 <= 20){
    if(distance_TR2 != 0 || distance_TL2 != 0  || distance_L2 != 0 || distance_R2 != 0  ){
      switch(gesture2){
        case 1:
...
void g(){
   ...
   else if(analogRead(A6)>600){
    Serial.println("model3");
     Serial.write(6);
    Serial.flush();
    delay(20);
  }
   //else if(analogRead(A7)>300){
